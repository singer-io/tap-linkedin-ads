...
class LinkedInAds:
    ...
    properties:
        ...
        account_filter       : Method for Account filtering. Each uses a different query pattern/parameter:
            search_id_values_param, accounts_param
...
class CampaignGroups(LinkedInAds):
    ...
    params = {
        ...
        "search": "(status:(values:List(DRAFT,ACTIVE,PAUSED,ARCHIVED,CANCELED,PENDING_DELETION,REMOVED)))"
    }
    headers = {'X-Restli-Protocol-Version': "2.0.0"}
...
class Campaigns(LinkedInAds):
    ...
    params = {
        "q": "search",
        "sort": "(field:ID,order:ASCENDING)",
        "search": "(status:(values:List(DRAFT,ACTIVE,PAUSED,ARCHIVED,CANCELED,PENDING_DELETION,REMOVED,COMPLETED)))"
    }
    headers = {'X-Restli-Protocol-Version': "2.0.0"}
