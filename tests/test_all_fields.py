    ...
    def test_run(self):
        ...
        for stream in expected_streams:
            with self.subTest(stream=stream):
                ...
                # As we Can't request more than 20 metrics for adanalytics hence some of the fields are missing: 
                # https://docs.microsoft.com/en-us/linkedin/marketing/integrations/ads-reporting/ads-reporting?tabs=http#requesting-specific-metrics-in-the-analytics-finder
                elif stream in ["ad_analytics_by_creative","ad_analytics_by_campaign"]:
                    expected_all_keys.remove('viral_landing_page_clicks')
                    expected_all_keys.remove('viral_reactions')
                    expected_all_keys.remove('viral_other_engagements')
                    expected_all_keys.remove('viral_likes')
                    expected_all_keys.remove('lead_generation_mail_contact_info_shares')
                    expected_all_keys.remove('lead_generation_mail_interest_clicks')
                    expected_all_keys.remove('viral_total_engagements')
                    expected_all_keys.remove('viral_impressions')
                    expected_all_keys.remove('viral_video_completions')
                    expected_all_keys.remove('viral_video_third_quartile_completions')
                    expected_all_keys.remove('viral_video_midpoint_completions')
                    expected_all_keys.remove('viral_one_click_lead_form_opens')
                    expected_all_keys.remove('viral_shares')
                    expected_all_keys.remove('viral_one_click_leads')
                    expected_all_keys.remove('viral_video_starts')
                    expected_all_keys.remove('viral_video_views')
                    expected_all_keys.remove('viral_video_first_quartile_completions')
